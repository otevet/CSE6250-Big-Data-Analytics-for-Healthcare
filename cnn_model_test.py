...
def parse_args():
    ...
    parser.add_argument('--datafile', dest='datafile', help='input pickle file',
                        default='./data/DATA_WORDSEQV1_HADM_TOP50.p', type=str)
    parser.add_argument('--embmatrix', dest='embmatrix', help='embedding matrix',
                        default='./data/EMBMATRIXV1_WORD2VEC_v2_300dim.p', type=str)
    parser.add_argument('--batch_size', dest='batch_size', help='batch size', default=128, type=int)
    parser.add_argument('--model_name', dest='model_name', help='model loaded from dl_model.py', default='cnn_text',
                        type=str)
    parser.add_argument('--append_name', dest='pre_train_append', help='load weights_model_name<append_name>',
                        default='', type=str)
    ...
    parser.add_argument('--eval_topN', dest='eval_topN', help='evaluate only the top N labels (ordered by f1 score)',
                        default=50, type=int)
    parser.add_argument('--eval_firstN', dest='eval_firstN', help='evaluate only the first N labels', default=-1,
                        type=int)
...
def test(args):
    ...
    model_func = getattr(cnn_models, model_name)
    ...
    for i in range(0, int(test_label.shape[1] / args.eval_everyN)):
